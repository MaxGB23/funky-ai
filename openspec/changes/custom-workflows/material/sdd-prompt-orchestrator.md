...
- **Topic update rules**: distintos topic keys NO deben overwritearse entre sí. Mismo topic evolucionando → mismo `topic_key` (upsert). Si no estás seguro del key, llamar `mem_suggest_topic_key`. Si sabés el ID exacto, usar `mem_update`.
