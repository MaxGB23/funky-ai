...
- [ ] Crear una carpeta vacía fuera de `m:\funky-ai\` (ej. `C:\test\funky-headless\`)
- [ ] Abrir una terminal en esa carpeta y ejecutar:
  ```
  funky init --template
  ```
- [ ] Validar que se generó un `PROJECT-CANVAS.md` con secciones vacías ("No definido")
- [ ] Editar manualmente el `PROJECT-CANVAS.md` con cualquier contenido inventado (al menos 2-3 secciones con texto propio)
- [ ] Ejecutar:
  ```
  funky init
  ```
...
- [ ] Crear otra carpeta vacía distinta (ej. `C:\test\funky-interactive\`)
- [ ] Abrir una terminal en esa carpeta y ejecutar:
  ```
  funky init
  ```
- [ ] Responder todos los prompts interactivos (patrón arquitectónico, UI framework, TDD)
- [ ] **Criterios de éxito:**
  - ✅ Los prompts se mostraron y respondieron correctamente
  - ✅ Se generó `PROJECT-CANVAS.md` con los valores elegidos en los prompts
  - ✅ Se creó toda la estructura de scaffolding (misma que el Escenario 1)
  - ✅ La experiencia de usuario (UX) fue fluida, sin mensajes de error ni stacktraces
