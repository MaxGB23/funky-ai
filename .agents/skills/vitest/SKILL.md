...
- **Assertion Strategy (Anti-Brittle Tests):** NEVER hardcode copy, exact wording or presentation strings (e.g. `expect(output).toContain('Exact Title')`, `expect(strings).toContain('Full Message')`, `expect(spy).toHaveBeenCalledWith('Exact Copy')`) when testing templates, console outputs or UI. Use **Snapshot Testing** (`toMatchSnapshot()`) for full outputs, or validate **Structure/Behavior** using semantic markers, tokens, and logic branches. Tests MUST survive non-functional copywriting changes.
- **Fragile-assertion gate (mechanical, enforced by `tests/organization.test.js`):** the gate counts `toContain`/`toMatch`/`.includes`/`stringContaining`/`stringMatching`/`toHaveBeenCalledWith` with a STRING LITERAL argument (single/double/backtick quotes; `toHaveBeenCalledWith` only with a sole string-literal arg). Console-copy assertions must use outcome/structural forms: `stringMatching(/semantic-token/)` (regex args are the structural semantic-token form — write tokens as regexes, not string literals), `toHaveBeenCalled()` plus exit-code/file outcomes instead of `toHaveBeenCalledWith('copy')`, or `toMatchSnapshot()` for full outputs. Backtick literals containing `${...}` (derived counts, computed data) are dynamic → non-fragile. Paths, filenames, CLI flags, markers and `{{TOKEN}}` placeholders are structural. Copy assigned to a variable is still copy-coupled — use the regex-token form, never a variable alias.
- **Compact vs Extract:** compact a test file when the near-cap pressure comes from *redundant assertions of the same contract* (merge/consolidate, never drop coverage); EXTRACT a new `{unit}.test.js` (or module) when the file is near the line cap because it holds *distinct responsibilities* (e.g. `estimateDomain.test.js` marker suites → `estimateMarkers.test.js`). Extraction must keep one direction of imports and update `tests/organization.test.js` maps accordingly.
- **Snapshot discipline:** template-file snapshots MUST normalize `\r\n` → `\n` before `toMatchSnapshot()` (a CRLF checkout must not destabilize CI). Generate snapshots locally with `pnpm test -u` per file, REVIEW the diff, then commit; NEVER run blind `-u` and NEVER `-u` in a CI context (CI=true fails on missing/obsolete snapshots instead of writing them).
- Enforcement: `tests/organization.test.js` (meta-test, runs with `pnpm test`) fails on rule violations; fragile-copy detection covers the literal forms listed above and the `FRAGILE_DEBT` map must stay empty — any new literal-form assertion fails the suite. Legacy debt is tracked in its `LEGACY_EXCEPTIONS` map (may be empty once debt is migrated) and must be removed when a file is migrated.
