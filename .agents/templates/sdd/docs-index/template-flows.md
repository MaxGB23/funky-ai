...
- **Árbol real de `funky-cli/src/templates/`:** Estado verificado de `assess/`, `bootstrap/`, `estimate/`, `init/` y `gentle/skills/`; lista explícita de lo que NO existe.
...
- **Flujo 4 — Skills (paquete → proyecto):** ✅ FUNCIONAL (2026-08-03, manifiesto 2026-08-04). `skills.js` inyecta las skills desde sus manifests (`src/skills/<skill>/`) a `.agents/skills/` y los docs compartidos (`bootstrap/sdd/docs-live-index.md` + `docs-index/_indice-seccional-template.md`) con paridad de bytes respecto a scaffold (R-SK-5).
