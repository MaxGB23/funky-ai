...
- **2. Tiers de inyección:** Selector interactivo gobernado por `INJECTION_MATRIX` y `resolveFiles()`.
  ...
  - **T2 — Feature / SDD ligero:** Base + `explore`, `proposal`, `spec`, `release.md` y `docs.md` condicional.
  - **T3 — Feature compleja / Archivo viviente:** `tasks.md`, `release.md` y `docs.md` condicional.
- **3. Golden templates vs Fallback:** Prioridad a `.agents/templates/sdd/`; fallback a `src/templates/sdd/` con warning.
