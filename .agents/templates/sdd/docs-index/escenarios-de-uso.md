...
- **Escenario 1:** "No tengo claro qué quiero construir" (Debate → `funky init` → Llenado → `funky scaffold`).
  - **"No tengo claro qué quiero construir":** Debate en chat vacío, canvases, assess opcional, scaffold y estimate.
- **Escenario 2:** "Sé qué quiero construir, arranco desde cero" (`funky init` → Llenado → `funky scaffold`).
  - **"Sé qué quiero construir, arranco desde cero":** Canvases vacíos, llenado, primer commit y arranque SDD.
- **Escenario 3:** "Tengo un repo existente sin Funky AI" (`funky init` → Llenado → `funky scaffold`).
