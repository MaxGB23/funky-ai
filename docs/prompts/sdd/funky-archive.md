...
2. Obtener el **feature path** como argumento de entrada (ej. `024-living-specs`). El path completo es `docs/openspec/changes/{feature}/`.
3. Listar `docs/openspec/changes/{feature}/specs/` para identificar los dominios a mergear.
4. `view_file docs/openspec/changes/{feature}/verify-report.md` — DEBE existir con status PASS. Si está en FAIL o no existe, abortar con error:
...
Para cada dominio encontrado en `docs/openspec/changes/{feature}/specs/`:

1. `view_file docs/openspec/changes/{feature}/specs/{domain}/spec.md` — leer el campo `root-sha256` del frontmatter/header.
...
1. `view_file docs/openspec/specs/{domain}/spec.md` — cargar el Root Spec completo.
...
3. Escribir el Root Spec resultante en `docs/openspec/specs/{domain}/spec.md`.
...
1. Verificar que `docs/openspec/specs/{domain}/` NO existe (es un dominio nuevo).
...
2. Crear el directorio `docs/openspec/specs/{domain}/` si no existe.
3. Copiar el contenido del Delta Spec íntegro a `docs/openspec/specs/{domain}/spec.md`, **eliminando** el header Delta (`# Delta for {Domain}`, `root-sha256`, etc.) y convirtiendo el documento a formato Root Spec:
...
2. Mover la carpeta completa:
   ```
   docs/openspec/changes/{feature}/  →  docs/openspec/archive/{new-name}/
   ```
3. Verificar el conteo de entradas en `docs/openspec/archive/`:
...
4. Confirmar que `docs/openspec/changes/{feature}/` ya no existe.

---

## Paso Final: Escribir Archive Report

Crear `docs/openspec/archive/{new-name}/archive-report.md`:
...
**Destino:** docs/openspec/archive/{new-name}/
...
**Artefacto:** docs/openspec/archive/{new-name}/archive-report.md
