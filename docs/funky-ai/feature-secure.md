...
6. **`funky secure` asume pnpm como gestor estándar.** Independiente de antigravity (no toca `funky sdd install` ni su `secops.md`), pero **no agnóstico de gestor**: el estándar es pnpm. Repos con otro gestor quedan fuera del estándar por definición.
