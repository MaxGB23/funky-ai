...
- Alcance por lote (caso canónico: `sdd-docs-sync`, que corre tras merge/push a main): delega después de commitear y pasa el rango (`origin/main..HEAD`) en el handoff; el subagente usa diffs de ese rango para scoping y atribución (qué drift es del lote vs pre-existente), pero su ancla de verdad es siempre el estado actual del código, nunca la historia ni los mensajes de commit.
