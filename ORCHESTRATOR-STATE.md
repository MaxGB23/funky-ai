...
- [ ] **Feature `testing-modernization` (Deuda Técnica Integral): Refactor de Aserciones y Extracción de Dominios.** Agrupa dos frentes críticos de testing que afectan a todo el proyecto (no solo a `estimate`):
  1. **Migración Anti-Brittle (Snapshots/Structural):** Refactorizar las >190 aserciones frágiles de copy estático (`expect().toContain()` en `templates`, `skills`, `secureYaml`, etc.) hacia `toMatchSnapshot()` o validaciones estructurales/marcadores, cumpliendo la nueva política de la skill de `vitest`.
