...
- **Estado:** 🟢 Publicada. Release v4.5.0 (rename-scaffold-sdd-install, PR #38): `funky sdd install` es el nombre canónico del framework completo (namespace `sdd` + subcomando `install`) y `funky scaffold` es un comando real agnóstico openspec/sdd que instala solo 4 archivos base (README interpolado, ORCHESTRATOR-STATE, release-notes→`.agents/templates/sdd/`, 000-rfc-template→`openspec/rfcs/`) — ya no es alias deprecado ni instala el ecosistema completo. Docs sync post-merge: `scaffold.md` re-creado y registrado en el SSOT con índice seccional; repo-map alineado. PR #38 mergeado (`b4353ca`).
