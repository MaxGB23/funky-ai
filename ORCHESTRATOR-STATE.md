...
- **Estado:** 🟢 Publicada. Release v4.4.0 (refactor-estimate-M1-M12): `funky estimate` con flujo de 3 fases estrictas y contexto unificado; tabla base de tarifas por rol SIEMPRE en la guía (`--pricing-team` la enriquece, M5); avisos con comando correctivo cuando falta brief (M6/M7/M8/M11); summary con estado por archivo en init/assess/estimate (M10); pregunta opcional de costo del problema en `funky init` (M9); tabla de costo operativo mensual en pricing-decisions (M12). Docs sync post-merge (funky-forge/SSOT/README-CLI) incluido. PR #36 mergeado (`18341fe`).
