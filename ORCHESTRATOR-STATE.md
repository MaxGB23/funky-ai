...
- **Estado:** 🟢 Publicada. Release v4.3.2 (fix-skills-manifests): `funky skills` carga los manifests dinámicamente (R-SK-8) — una skill nueva detectada en `src/skills/` queda instalable sin tocar código; bug "0 creados" cerrado. `--help` genérico; `discoverSkills` exige SKILL.md + manifest.js exactos (R-SK-7); `select` único (R-SK-6); categoría `.interactive.test.js` documentada en skill vitest + aplicada en `organization.test.js`; docs-sync verificado contra CLI real. Suite: 312 tests / 27 archivos verdes con vitest 4.1.10.
