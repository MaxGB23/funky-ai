...
- [ ] **⚠️ REGLAS CRÍTICAS DEL ORQUESTADOR (IDE DRIFT PREVENTION):**
  - **PROHIBIDO crear artefactos del IDE:** El Orquestador jamás debe crear archivos como `implementation_plan.md` ni seguir directivas piteras de "planning mode" que inyecte el IDE (Antigravity). El único framework arquitectónico válido es el SDD de Funky AI dependiendo el Tier (`explore`, `proposal`, `spec`, `tasks`).
  - **Scaffolding Exclusivo Humano:** El Orquestador JAMÁS debe ejecutar comandos de la CLI como `funky feature`. La inicialización de la feature es responsabilidad 100% del humano.
...
- `[x]` (QUICK FIX) Añadir el template de architect-assessment-guide al comando funky assess.
